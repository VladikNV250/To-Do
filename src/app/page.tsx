...
export default function Home() {
  ...
  const [navLists, setNavLists] = useState<List[]>([ // obligate navigation lists
    new List("My Day",         "gray",  [], "sun"),
    new List("Important",      "red",   [], "star"),
    new List("Planned",        "green", [], "plan", true),
    new List("Assigned to me", "cyan",  [], "person"),
    new List("Tasks",          "blue",  [], "house"),
  ]);
  const [lists, setLists] = useState<(List | Group)[]>([ // other custom lists
    new List("Everyday Things after school", "green", []),
    new List("Pet Projects", "red", []),
    new Group("NMT", [
      new List("English | NMT", "blue", []),
      new List("Math | NMT", "blue", []),
      new List("History | NMT", "blue", []),
      new List("Ukrainian | NMT", "blue", []),
    ]),
    new Group("Second group"),
  ]);
  const currentList = useRef<List | null>(getSelectedList());

  
  function getSelectedList(): List | null {
    let result: List | null = null;
    navLists.forEach(list => {if (list.selected === true) result = list; })
    lists.forEach(item => {
      if (item instanceof List)
        if (item.selected === true) result = item;
      if (item instanceof Group) 
        item.lists?.forEach(list => {if (list.selected === true) result = list; })
    })
    return result;
  }
    ...
    <>
      ...
      >
        <header className="fixed top-0 left-0 w-full flex-between pt-10 px-[45px] pb-4 bg-red-500/50 backdrop-blur z-10" >
          <TitleInput list={currentList.current} />
